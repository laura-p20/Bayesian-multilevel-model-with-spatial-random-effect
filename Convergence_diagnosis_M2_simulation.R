...
setwd("/Users/andresandradevargas/Desktop/Cosas_LAU")
...
load("~/Desktop/Cosas_LAU/data_ready.RData")
...
load("~/Desktop/Cosas_LAU/simulation_response_scene_1.RData")
load("~/Desktop/Cosas_LAU/simulation_response_scene_2.RData")
load("~/Desktop/Cosas_LAU/simulation_response_scene_3.RData")

y_ijk_scenes<-cbind(y_ijk_1,y_ijk_2,y_ijk_3)
#Extracting the scenarios values 

load("~/Desktop/Cosas_LAU/simulation_values.RData")

##===========Extracting the parameter of the samples----

# Scenario 1
load("MCMC_model2_simulated_e1.RData")
...
alpha_kappa1<-THETA[[13]]
beta_kappa1<-THETA[[14]]

acr1<-THETA[[15]]

#Scenario 2
load("MCMC_model2_simulated_e2.RData")
...
alpha_kappa2<-THETA[[13]]
beta_kappa2<-THETA[[14]]

acr2<-THETA[[15]]

load("MCMC_model2_simulated_e3.RData")
...
alpha_kappa3<-THETA[[13]]
beta_kappa3<-THETA[[14]]
...
#Calculus of the real log-likelyhoods
...
par(mfrow=c(1,3))
plot(ll_1,pch = 20, cex = 0.45,xlab = "",ylab = "Log likelihood",main="Scenario 1",col="#70284a",col.main="#70284a",ylim = c(-1232970,-1232150))
abline(h=mean(ll_true[1,1]),col="#A0153E",lwd="2")
plot(ll_2,pch = 20, cex = 0.45,xlab = "Iteration",ylab = "",main="Scenario 2",col="#dc7176",col.main="#70284a",ylim = c(-1232970,-1232150))
abline(h=mean(ll_true[1,2]),col="#A0153E",lwd="2")
plot(ll_3,pch = 20, cex = 0.45,xlab = "",ylab = "",main="Scenario 3",col="#f2a28a",col.main="#70284a",ylim = c(-1232970,-1232150))
...
neff_sigma2_E<-round(coda::effectiveSize(sigma2_E1), 0)
neff_sigma2_M<-round(coda::effectiveSize(sigma2_M1), 0)
neff_sigma2_D<-round(coda::effectiveSize(sigma2_D1), 0)
...
neff_sigma2_E<-round(coda::effectiveSize(sigma2_E2), 0)
neff_sigma2_M<-round(coda::effectiveSize(sigma2_M2), 0)
neff_sigma2_D<-round(coda::effectiveSize(sigma2_D2), 0)
...
neff_sigma2_E<-round(coda::effectiveSize(sigma2_E3), 0)
neff_sigma2_M<-round(coda::effectiveSize(sigma2_M3), 0)
neff_sigma2_D<-round(coda::effectiveSize(sigma2_D3), 0)
...
load("~/Desktop/Tesis/Results/PPP_basic_simulation.RData")

ggplot(PPP_basic_simulation$municipal_PPP_boxplot_df, aes(x = scenario, y = PPP_mean, fill = scenario, colour = scenario)) +
  geom_boxplot(
...
ggplot(PPP_basic_simulation$municipal_PPP_boxplot_df, aes(x = scenario, y = PPP_var, fill = scenario, colour = scenario)) +
  geom_boxplot(
    alpha = 0.7,          # Transparencia media
    outlier.shape = NA   # Ocultamos outliers  # Borde gris oscuro uniforme
  ) +
...
tapply(PPP_basic_simulation$departamental_PPP_boxplot_df, PPP_basic_simulation$departamental_PPP_boxplot_df$scenario, summary)
...
RMSE_MAE_summary$others<-data.frame(
  ...
  municipal_variance=c(mean(abs(apply(kappa2_jk1,2,mean)-scenarios_parameter$kappa2_jk_post)),
          mean((apply(kappa2_jk1,2,mean)-scenarios_parameter$kappa2_jk_post)^2)),
  spatial_effect=c(mean(abs(apply(phi1,2,mean)-scenarios_parameter$phi_post)),
                   mean((apply(phi1,2,mean)-scenarios_parameter$phi_post)^2))
)

##=========DIC==========
###-------scenario 1---------

mean_est_bayes_1<-mean(beta1)+X_ijk%*%apply(beta_E1,2,mean)+Z_jk%*%apply(beta_M1,2,mean)+W_k%*%apply(beta_D1,2,mean)+rep(apply(phi1,2,mean),n_jk)
...
mean_est_bayes_2<-mean(beta2)+X_ijk%*%apply(beta_E2,2,mean)+Z_jk%*%apply(beta_M2,2,mean)+W_k%*%apply(beta_D2,2,mean)+rep(apply(phi2,2,mean),n_jk)
...
mean_est_bayes_3<-mean(beta3)+X_ijk%*%apply(beta_E3,2,mean)+Z_jk%*%apply(beta_M3,2,mean)+W_k%*%apply(beta_D3,2,mean)+rep(apply(phi3,2,mean),n_jk)
...
save(DIC_WAIC,file="DIC_WAIC_basic.RData")
