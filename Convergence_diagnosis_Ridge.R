...
load("~/Desktop/Tesis/Procesed_Data/simulation_values.RData")

##===========Extracting the parameter of the samples----

# Scenario 1
load("~/Desktop/Tesis/Results/MCMC_ridge_simulated_e1.RData")
...
#Scenario 2
load("~/Desktop/Tesis/Results/MCMC_ridge_simulated_e2.RData")
...
load("~/Desktop/Tesis/Results/MCMC_ridge_simulated_e3.RData")
...
save(ll,file="ll_simulation_Ridge.RData")
...
neff_lambda2_E<-round(coda::effectiveSize(lambda2_E1), 0)
neff_lambda2_M<-round(coda::effectiveSize(lambda2_M1), 0)
neff_lambda2_D<-round(coda::effectiveSize(lambda2_D1), 0)
...
neff_lambda2_E<-round(coda::effectiveSize(lambda2_E2), 0)
neff_lambda2_M<-round(coda::effectiveSize(lambda2_M2), 0)
neff_lambda2_D<-round(coda::effectiveSize(lambda2_D2), 0)
...
neff_lambda2_E<-round(coda::effectiveSize(lambda2_E3), 0)
neff_lambda2_M<-round(coda::effectiveSize(lambda2_M3), 0)
neff_lambda2_D<-round(coda::effectiveSize(lambda2_D3), 0)
...
df1<- data.frame(
  ...
  media = c(mean(beta1),apply(beta_E1,2,stat_mode),apply(beta_M1,2,stat_mode),apply(beta_D1,2,stat_mode)),
...
df2<- data.frame(
  ...
  media = c(mean(beta2),apply(beta_E2,2,stat_mode),apply(beta_M2,2,stat_mode),apply(beta_D2,2,stat_mode)),
...
df3<- data.frame(
  ...
  media = c(mean(beta3),apply(beta_E3,2,stat_mode),apply(beta_M3,2,stat_mode),apply(beta_D3,2,stat_mode)),
...
for(i in c(1:nsams)){
  ...
  departamental_mean1<-tapply(y_ppp,index_dep,mean)
  departamental_variance_mean1<-tapply(y_ppp,index_dep,var)
  ...
  departamental_mean2<-tapply(y_ppp,index_dep,mean)
  departamental_variance_mean2<-tapply(y_ppp,index_dep,var)
  ...
  departamental_mean3<-tapply(y_ppp,index_dep,mean)
  departamental_variance_mean3<-tapply(y_ppp,index_dep,var)
...
PPP_ridge<-list(municipal_PPP_boxplot_df=municipal_PPP_boxplot_df,departamental_PPP_boxplot_df=departamental_PPP_boxplot_df)
save(PPP_ridge,file="PPP_ridge_simulations.RData")
...
summary(subset(departamental_PPP_boxplot_df,scenario=="1"))
summary(subset(departamental_PPP_boxplot_df,scenario=="2"))
summary(subset(departamental_PPP_boxplot_df,scenario=="3"))
...
save(DIC_WAIC,file="DIC_WAIC_Ridge_simulation.RData")
